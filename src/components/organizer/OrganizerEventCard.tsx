...
import { Calendar, Users, Edit, Trash2, Award, Bell, UserPlus, CircleDollarSign, Eye, RefreshCw, AlertTriangle, Palette } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  const renderActions = () => {
    ...
    switch(displayStatus) {
      ...
      case 'approved':
         return (
           <>
             <Button asChild size="sm"><Link href={`/organizer/event/${event.id}/registrations`}>Manage</Link></Button>
             <Button asChild variant="outline" size="sm" className="bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100">
               <Link href={`/organizer/event/${event.id}/certificates`}>
      ...
      case 'completed':
          return <Button asChild variant="outline" size="sm"><Link href={`/organizer/event/${event.id}/certificates`}><Award className="mr-2 h-4 w-4" />Issue Certificates</Link></Button>
      default:
        return null;
    }
  }
             ...
             {(displayStatus === 'approved' || displayStatus === 'completed') && (
                <>
                    ...
                    {displayStatus === 'approved' && (
                      <DropdownMenuItem asChild>
                        <Link href={`/organizer/reschedule/${event.id}`}>
                          <RefreshCw className="mr-2 h-4 w-4"/>Reschedule
                        </Link>
                      </DropdownMenuItem>
                    )}
                    <DropdownMenuItem asChild>
                      <Link href={`/organizer/event/${event.id}/volunteers`}>
                        <UserPlus className="mr-2 h-4 w-4" />Volunteers
                      </Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild>
                      <Link href={`/organizer/event/${event.id}/certificate-designer`}>
                        <Palette className="mr-2 h-4 w-4" />Certificate Designer
                      </Link>
                    </DropdownMenuItem>
                    {event.isPaid && (
                      <DropdownMenuItem asChild>
                        <Link href={`/organizer/event/${event.id}/budget`}>
                          <CircleDollarSign className="mr-2 h-4 w-4" />Budget
                        </Link>
                      </DropdownMenuItem>
                    )}
                    <DropdownMenuItem>
      ...
      <CardContent className="flex-1 space-y-2 pt-4">
        ...
        <div className="space-y-1 text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            ...
            <span>{new Date(event.date).toLocaleDateString('en-IN', { dateStyle: 'medium' })} @ {event.startTime}</span>
          </div>
          <div className="flex items-center gap-2">
