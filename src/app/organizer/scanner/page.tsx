      ...
      {selectedEventId && (
        <>
          ...
          <Card>
             ...
             <CardContent>
                <div className={`relative w-full max-w-md mx-auto rounded-xl overflow-hidden ${!isScanning ? 'hidden' : ''}`} style={{ minHeight: '300px' }}>
                  {/* Dedicated mount point for Html5Qrcode to avoid React hydration issues */}
                ...
                {!isScanning && !scanResult && !loading && (
                 ...
                 {loading && (
                  <div className="text-center py-12"><Loader2 className="h-16 w-16 animate-spin mx-auto text-primary" /><p className="mt-4 text-muted-foreground">Verifying...</p></div>
                )}
                 {scanResult && !loading && (
