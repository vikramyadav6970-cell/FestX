...
import { Loader2, Calendar, MapPin, Users, CircleDollarSign, CheckCircle } from 'lucide-react';
...
const generateSchema = (fields: EventType['formFields'] = []) => {
  ...
  const schemaObject = safeFields.reduce((acc, field) => {
    if (!field || !field.label) return acc;
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const registrationData = {
        eventId: event.id,
        organizerId: event.organizerId, // Added for security rules access
